...
/// @title Staked Canto
///
/// @dev This is the contract for the Liquid Staked Canto token
/// The role admin must give a minting contract permisions required to mint
/// tokens.
contract StakedCanto is AccessControlUpgradeable, ERC20Upgradeable {
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "StakedCanto: only minter");
        _;
    }

    /// @dev A modifier which checks that the caller has the burner role.
    modifier onlyBurner() {
        require(hasRole(BURNER_ROLE, msg.sender), "StakedCanto: only burner");
        _;
    }
    ...
    function initialize(
        ...
        ERC20Upgradeable.__ERC20_init("Liquid Staked Canto", "sCANTO");
